...
@Component({
  ...
  styleUrl: './forhim.component.css',
  encapsulation: ViewEncapsulation.None
})
export class ForhimComponent implements OnInit, AfterViewInit {
  ...
  // Doctors filtered for male patients
  doctors: any[] = [];
  ...
  private loadServices(): void {
    ...
    this.supabaseService.getAllServices().subscribe({
      next: (services: any[]) => {
        console.log('📥 Raw services from Supabase:', services.length);
        console.log('📥 First 3 services:', services.slice(0, 3).map((s: any) => ({
          id: s.id,
          name: s.name,
          title: s.title
        })));
        ...
        console.log('✅ Loaded services map:', this.servicesMap.size, 'services');
        console.log('🗺️ Sample entries from map:');
        let count = 0;
  ...
  private loadDoctors(): void {
    ...
    this.supabaseService.getAllDoctors().subscribe({
      next: (doctors: any[]) => {
        console.log('📦 For Him - Raw doctors from Supabase:', doctors?.length || 0);

        // Log first doctor's services to see format
        if (doctors && doctors.length > 0 && doctors[0].services) {
          console.log('🔍 First doctor services (raw):', doctors[0].name, '→', doctors[0].services);
        }
        ...
        if (doctors && doctors.length > 0) {
          ...
          // If forhim_doctors_featured array exists and has items, filter by it
          console.log('🔍 For Him - forhim_doctors_featured from Supabase:', this.pageContent.forhim_doctors_featured);
          console.log('🔍 For Him - Featured array length:', this.pageContent.forhim_doctors_featured?.length || 0);
          console.log('🔍 For Him - All active doctors count:', allDoctors.length);
          ...
          if (this.pageContent.forhim_doctors_featured && this.pageContent.forhim_doctors_featured.length > 0) {
            this.doctors = allDoctors.filter(d =>
              this.pageContent.forhim_doctors_featured.includes(d.id)
            ) as any[];
            console.log('✅ For Him - Filtered to featured doctors:', this.doctors.length);
            console.log('👥 For Him - Doctor names showing:', this.doctors.map(d => d.name));
            if (this.doctors.length === 0) {
              console.error('⚠️ For Him - No doctors match the featured IDs! The IDs in forhim_doctors_featured dont exist in Supabase.');
              console.error('   Featured IDs:', this.pageContent.forhim_doctors_featured);
            ...
            console.log('⚠️ For Him - No doctors selected in admin. Array is empty or null.');
          }
        } else {
          console.error('❌ For Him - No doctors returned from Supabase');
          this.doctors = [];
        }
      },
      error: (err) => {
  ...
  private cleanupDemoIds(): void {
    ...
    if (this.pageContent.forhim_doctors_featured && Array.isArray(this.pageContent.forhim_doctors_featured)) {
      const hasDemoIds = this.pageContent.forhim_doctors_featured.some((id: string) => id.match(/^doc_\d+$/));

      if (hasDemoIds) {
        console.log(`🧹 For Him Frontend - Clearing ${this.pageContent.forhim_doctors_featured.length} demo IDs. Admin needs to select doctors.`);
        this.pageContent.forhim_doctors_featured = [];
      }
    }
  }
  ...
  private mapServiceIdsToNames(serviceIds: string[]): string[] {
    ...
    console.log('🔍 Mapping service IDs:', serviceIds);
    console.log('🗺️ Services map size:', this.servicesMap.size);
    ...
    const mapped = serviceIds
      .map(id => {
        ...
        if (!isUUID) {
          ...
          console.log(`  "${id}" → Already a name (not UUID)`);
          return id;
        }
        ...
        console.log(`  ID: ${id} → NOT FOUND in services map, filtering out`);
        return null;
      })
  ...
  private initLeafletMap(): void {
    ...
    if (BR.length === 0) {
      console.warn('⚠️ No map_branches found in page content. Map will be empty.');
      return;
    }
  ...
  private initContactForm(): void {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        // Save booking to Supabase (await completion before continuing)
        await new Promise<void>((resolve, reject) => {
          this.bookingsService.createBooking({
            source: 'forhim_form',
            page_source: 'For Him Page',
        ...
        await this.googleSheetsService.sendToGoogleSheets(formData, 'forhim');
  ...
  private initBookingLinks(): void {
    // Read booking link from Supabase page content - NO HARDCODED FALLBACK
    const BK = this.pageContent?.booking_link || '';

    if (!BK) {
      console.warn('⚠️ No booking_link found in page content. Booking buttons will not have links.');
      return;
    }
